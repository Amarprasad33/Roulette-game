...
import { GameState, BettingNumber, OutgoingMessages } from "@repo/common/types";
...
export default function AdminMain({password}: {password: string}) {
  const {socket, loading} = useSocket(password);

  const numbers: BettingNumber[] = Array.from({ length: 37 }, (_, i) => i);
  ...
  function handleSelect({number}: any) {
